...
trait HServer {
  ...
  protected def startStopListener : Unit = tiscaf.sync.Sync.spawnNamed("StopListener") {
  ...
  private lazy val talksExe = new tiscaf.sync.SyncExe(poolSize, queueSize, "ServerExecutorPool", onError)
  private val timeoutMillis = connectionTimeoutSeconds * 1000L
  private val isStopped = new tiscaf.sync.SyncBool(true)
