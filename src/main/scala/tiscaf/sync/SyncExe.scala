...
final class SyncExe(poolSize: Int, queueCap: Int, name: String, onError: (String, Exception) => Unit = {(_, _) =>  }) {
  ...
  private def newThread(i : Int) : Thread = new Thread {
    override def run = while (working.get) {
      try {
        ...
        case e : Exception =>
          self.onError("Some error occurred while processing thread " + name + "-" + poolNum + "-" + i, e)
      }
    }
